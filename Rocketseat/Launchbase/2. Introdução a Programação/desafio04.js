...
function getHigherTransactionByType (typeTransaction) {
    ...
    for (let transactionUser of user.transactions) {
        if ( transactionUser.type == typeTransaction  &&
             transactionUser.value > value ) {
            value = transactionUser.value
            result = { type: transactionUser.type, value: transactionUser.value }
        }
    }
...
function getAverageTransactionValue() {
    ...
    for (let transactionUser of user.transactions) {
        valueTot = valueTot + transactionUser.value
        countRegister++
    }

    if (valueTot > 0) {
        result = valueTot / countRegister
    }
...
function getTransactionsCount() {
    ...
    for (let userTransaction of user.transactions) {
        if (userTransaction.type == 'credit'){
            countCredit++
        } else {
            countDebit++
        }
    ...
    const result = {credit: countCredit, debit: countDebit}
